...
function applyHighlightColour(
  ...
  function resetTextColour() {
    ...
    /*
     * execCommand("foreColor", "inherit") is not a reliable reset.
     * Chromium can interpret that invalid colour value unpredictably.
     *
     * Instead:
     * 1. colour ONLY the current selection with a temporary sentinel;
     * 2. find the nodes created/changed by that operation;
     * 3. remove only their explicit colour declaration;
     * 4. leave every other inline style (bold/italic/font-size/etc.) intact.
     *
     * With no explicit colour left, the text inherits the editor's real
     * default colour from the active skin.
     */
    const sentinelHex = "#010203";
    const sentinelRgb = "rgb(1, 2, 3)";
    ...
    editor
      ...
      .forEach((node) => {
        ...
        const isSentinel =
          fontColour === sentinelHex ||
          inlineColour === sentinelHex ||
          inlineColour === sentinelRgb;
