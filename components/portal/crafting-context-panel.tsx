...
export function CraftingContextPanel() {
  ...
  if (!cancelled) {
    setError(characterError?.message ?? "Unable to identify your character.");
    setLoading(false);
  }
  return;
}
...
async function refresh() {
        ...
        if (knownResult.error || inventoryResult.error) {
          setError(knownResult.error?.message ?? inventoryResult.error?.message ?? "Unable to load crafting context.");
          setLoading(false);
          return;
        }

        const recipeIds = (knownResult.data ?? []).map((row) => String(row.recipe_id));
        ...
        if (recipeResult.error || ingredientResult.error) {
          setError(recipeResult.error?.message ?? ingredientResult.error?.message ?? "Unable to load crafting context.");
          setLoading(false);
          return;
        }
...
channel.subscribe();
  ...
  const summary = useMemo(() => {
    ...
    const statuses: RecipeStatus[] = recipes.map((recipe) => {
      const requirements = ingredientsByRecipe.get(recipe.id) ?? [];
      let missingUnits = 0;
      let missingKinds = 0;

      for (const requirement of requirements) {
        const owned = inventoryByItem.get(requirement.ingredient_item_id) ?? 0;
        const missing = Math.max(0, requirement.quantity - owned);
    ...
    const usedIngredientIds = new Set(
      ingredients.map((ingredient) => ingredient.ingredient_item_id),
    );

    const materialUnitsCarried = [...usedIngredientIds].reduce(
      (total, itemId) => total + (inventoryByItem.get(itemId) ?? 0),
      0,
    );
    ...
    for (const ingredient of ingredients) {
      const owned = inventoryByItem.get(ingredient.ingredient_item_id) ?? 0;
      if (owned < ingredient.quantity) {
        missingIngredientKinds.add(ingredient.ingredient_item_id);
      }
    }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      {error ? (
        <p className="mt-3 text-[10px] leading-5 text-red-400">{error}</p>
      ) : null}
        ...
        <div className="mt-3 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
          <section className="border border-[rgb(var(--sep-global-c1))]/35 bg-[rgb(var(--sep-colour-100c09))] p-3">
            ...
            <div className="mt-2 grid grid-cols-2 gap-px overflow-hidden border border-[rgb(var(--sep-global-c1))]/25 bg-[rgb(var(--sep-global-c1))]/20">
              <div className="bg-[rgb(var(--sep-colour-15100d))] px-2.5 py-2">
                <p className="text-[7px] uppercase tracking-[0.12em] text-[rgb(var(--sep-global-c1))]">Recipes Known</p>
                <p className="mt-1 font-serif text-lg text-[rgb(var(--sep-global-c2))]">{recipes.length}</p>
              </div>

              <div className="bg-[rgb(var(--sep-colour-15100d))] px-2.5 py-2">
                <p className="text-[7px] uppercase tracking-[0.12em] text-[rgb(var(--sep-global-c1))]">Craftable Now</p>
                <p className="mt-1 font-serif text-lg text-[rgb(var(--sep-global-c2))]">{summary.craftableNow}</p>
              </div>

              <div className="bg-[rgb(var(--sep-colour-15100d))] px-2.5 py-2">
                <p className="text-[7px] uppercase tracking-[0.12em] text-[rgb(var(--sep-global-c1))]">Materials Carried</p>
                <p className="mt-1 font-serif text-lg text-[rgb(var(--sep-global-c2))]">{summary.materialUnitsCarried}</p>
              </div>

              <div className="bg-[rgb(var(--sep-colour-15100d))] px-2.5 py-2">
                <p className="text-[7px] uppercase tracking-[0.12em] text-[rgb(var(--sep-global-c1))]">Missing Materials</p>
                <p className="mt-1 font-serif text-lg text-[rgb(var(--sep-global-c2))]">{summary.missingIngredientKinds}</p>
              </div>
            </div>
          </section>
          ...
          <section className="border border-[rgb(var(--sep-global-c1))]/35 bg-[rgb(var(--sep-colour-100c09))] p-3">
            <div className="flex items-end justify-between gap-3">
              <div>
                <p className="text-[7px] uppercase tracking-[0.18em] text-[rgb(var(--sep-global-c1))]">Workbench Notes</p>
                <h3 className="mt-1 font-serif text-[15px] text-[rgb(var(--sep-global-c2))]">Closest to Crafting</h3>
              </div>
            ...
            <div className="mt-2 space-y-1.5">
              {workbenchRecipes.length ? (
                workbenchRecipes.map((recipe) => (
                  <div
                    key={recipe.id}
                    className="flex items-center justify-between gap-2 border border-[rgb(var(--sep-global-c1))]/25 bg-[rgb(var(--sep-colour-15100d))] px-2.5 py-2"
                  >
                    ...
                    <span
                      className={[
                        "shrink-0 text-[7px] uppercase tracking-[0.1em]",
                        recipe.ready
                          ? "text-emerald-400"
                          : "text-[rgb(var(--sep-global-c1))]",
                      ].join(" ")}
                    >
                      {recipe.ready ? "Ready" : `Missing ${recipe.missingUnits}`}
                    </span>
                  </div>
                ))
              ) : (
          ...
          <section className="border border-[rgb(var(--sep-global-c1))]/35 bg-[rgb(var(--sep-colour-100c09))] p-3">
            <p className="text-[7px] uppercase tracking-[0.18em] text-[rgb(var(--sep-global-c1))]">Crafting Notes</p>

            <p className="mt-2 text-[10px] leading-5 text-[rgb(var(--sep-global-c2))]">
              Recipes are learned via Recipe Items. Ingredients are taken from materials carried by your character when you craft.
            </p>
          </section>
        </div>
      )}
