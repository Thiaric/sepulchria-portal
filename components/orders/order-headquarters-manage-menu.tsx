...
function Colour({
  ...
  return (
    <label className="grid gap-1">
      <span className="text-[7px] uppercase tracking-[0.13em] text-[#806b50]">
        {label}
      </span>
      <input
        type="color"
        name={name}
        defaultValue={value}
        className="h-8 w-full border border-[#60482e]/55 bg-transparent"
      />
    </label>
  );
}
...
export function OrderHeadquartersManageMenu({
  ...
  return (
    <details className="relative col-span-2 sm:col-auto">
      <summary className="flex cursor-pointer list-none items-center justify-center border border-[#725c3d] bg-[#21190f] px-2 py-1.5 text-[8px] uppercase tracking-[0.12em] text-[#d6bb8d] transition hover:border-[#a17a49] hover:bg-[#352718] hover:text-[#f0d6a7] sm:px-3 sm:text-[9px] sm:tracking-[0.18em] [&::-webkit-details-marker]:hidden">
        Manage Headquarters
      </summary>

      <div className="absolute right-0 top-full z-[120] mt-2 max-h-[72vh] w-[min(92vw,560px)] overflow-y-auto border border-[#80613b] bg-[#15100d] p-4 text-left shadow-2xl">
        ...
        <h3 className="mt-1 font-serif text-lg text-[#dfc99f]">
          {data.orderName}
        </h3>

        {data.canInvite ? (
          <section className="mt-4 border-t border-[#60482e]/30 pt-3">
            <p className="text-[8px] uppercase tracking-[0.16em] text-[#8c704b]">
            ...
            <form action={inviteOrderHeadquarters} className="mt-2 grid gap-2">
              ...
              <select
                ...
                required
                defaultValue=""
                className="border border-[#60482e]/55 bg-[#100c09] px-2 py-2 text-xs text-[#d7c4a5]"
              >
              ...
              <textarea
                ...
                className="resize-y border border-[#60482e]/55 bg-[#100c09] px-2 py-2 text-xs text-[#d7c4a5]"
              />
            ...
            {data.externalGuests.length > 0 ? (
              <div className="mt-3 space-y-1.5">
                {data.externalGuests.map((g: any) => (
                  <div
                    key={g.invitationId}
                    className="flex items-center justify-between gap-3 border border-[#59432c]/35 bg-[#100c09] px-2.5 py-2"
                  >
                    <div className="min-w-0">
                      <p className="truncate text-xs text-[#c9b79a]">{g.name}</p>
                      <p className="mt-0.5 text-[7px] uppercase tracking-[0.12em] text-[#756958]">
                        {g.status}
                        {g.expiresAt
                    ...
                    <form action={revokeOrderHeadquartersGuest}>
                      ...
                      <button
                        type="submit"
                        className="text-[7px] uppercase tracking-[0.13em] text-[#d18f83]"
                      >
        ...
        {data.canCustomize ? (
          <details className="mt-4 border-t border-[#60482e]/30 pt-3">
            <summary className="cursor-pointer text-[8px] uppercase tracking-[0.16em] text-[#8c704b]">
              Location appearance
            </summary>
            ...
            <form action={updateOrderHeadquartersPresentation} className="mt-3 grid gap-2">
              ...
              <p className="text-[7px] leading-4 text-[#6f6252]">
