...
export async function OrderLevelStructure({ orderId }: { orderId: string }) {
  ...
  return (
    <section className="mt-8 border-t border-[#60482e]/35 pt-6">
      ...
      <h4 className="mt-1 font-serif text-2xl text-[#dec69a]">Levels &amp; Roles</h4>
              ...
              <div className="border-t border-[#59432c]/35 p-4">
                      ...
                      <div key={job.id} className="border border-[#59432c]/40 bg-[#15100d] p-4">
                        <form action={updateOrderJob}>
                          ...
                          <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_100px_auto]">
                            <div className="grid gap-2">
                              <input type="text" name="name" required maxLength={120} defaultValue={job.name} className="w-full border border-[#60482e]/50 bg-[#100c09] px-3 py-2 font-serif text-base text-[#d7c4a5]" />
                              <input type="text" name="description" defaultValue={job.description ?? ""} placeholder="Optional description" className="w-full border border-[#60482e]/40 bg-[#100c09] px-3 py-2 text-[10px] text-[#a99b89]" />
                            </div>
                            <input type="number" name="sortOrder" defaultValue={job.sort_order} className="w-full border border-[#60482e]/50 bg-[#100c09] px-2 py-2 text-center text-sm text-[#d7c4a5]" />
                            <div className="flex items-end gap-2">
                              <button type="submit" className="border border-[#765937]/55 bg-[#261b12] px-3 py-2 text-[8px] uppercase text-[#ccb083]">Save Role</button>
                          ...
                          <p className="mt-4 text-[8px] uppercase tracking-[0.18em] text-[#806b50]">Attribute modifiers</p>
                          <div className="mt-2 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                            {ATTRIBUTE_FIELDS.map(({ key, label }) => (
                              <label key={key}>
                                <span className="mb-1 block text-[7px] uppercase text-[#756958]">{label}</span>
                                <input type="number" name={`${key}Modifier`} min={-10} max={10} defaultValue={job[`${key}_modifier` as keyof ModifierFields]} className="w-full border border-[#60482e]/55 bg-[#100c09] px-2 py-2 text-center text-sm text-[#d7c4a5]" />
                              </label>
                            ))}
                          </div>
                        </form>
                        ...
                        <div className="mt-4 border-t border-[#59432c]/30 pt-4">
                          ...
                          <div className="mt-2 grid gap-3 md:grid-cols-2">
                            <div>
                              <p className="text-[8px] text-[#756958]">From lower Level</p>
                              <div className="mt-1 flex flex-wrap gap-1.5">
                                {incoming.length ? incoming.map((link) => {
                                  const source = jobById.get(link.from_job_id);
                                  return source ? <span key={link.id} className="border border-[#60482e]/40 px-2 py-1 text-[8px] text-[#b49a75]">L{source.level} · {source.name}</span> : null;
                                }) : <span className="text-[9px] italic text-[#665b4c]">No incoming links.</span>}
                              </div>
                            </div>
                            <div>
                              <p className="text-[8px] text-[#756958]">To higher Level</p>
                              <div className="mt-1 space-y-2">
                                {outgoing.map((link) => {
                                  const target = jobById.get(link.to_job_id);
                                  return target ? (
                                    <form key={link.id} action={deleteOrderJobLink} className="flex items-center justify-between gap-2 border border-[#60482e]/35 px-2 py-1.5">
                                      <input type="hidden" name="orderId" value={orderId} />
                                      <input type="hidden" name="linkId" value={link.id} />
                                      <span className="text-[8px] text-[#b49a75]">L{target.level} · {target.name}</span>
                                ...
                                {nextLevel?.jobs?.length ? (
                                  <form action={createOrderJobLink} className="flex gap-2">
                                    ...
                                    <select name="toJobId" required defaultValue="" className="min-w-0 flex-1 border border-[#60482e]/50 bg-[#100c09] px-2 py-2 text-[9px] text-[#d7c4a5]">
                                    ...
                                    <button type="submit" className="border border-[#765937]/55 bg-[#261b12] px-3 py-2 text-[7px] uppercase text-[#ccb083]">Link</button>
                                  </form>
                                ) : null}
                              </div>
                            </div>
                ...
                <form action={createOrderJob} className="mt-4 border border-dashed border-[#765937]/40 bg-[#15100d]/60 p-3">
                  ...
                  <div className="mt-3 grid gap-2 lg:grid-cols-[minmax(0,1fr)_90px_auto]">
                    <div className="grid gap-2">
                      <input type="text" name="name" required maxLength={120} placeholder="Role name" className="w-full border border-[#60482e]/50 bg-[#100c09] px-3 py-2 text-sm text-[#d7c4a5]" />
                      <input type="text" name="description" placeholder="Optional description" className="w-full border border-[#60482e]/40 bg-[#100c09] px-3 py-2 text-[10px] text-[#a99b89]" />
                    </div>
                    <input type="number" name="sortOrder" defaultValue={(level.jobs?.length ?? 0) * 10} className="w-full border border-[#60482e]/50 bg-[#100c09] px-2 py-2 text-center text-sm text-[#d7c4a5]" />
                    <button type="submit" className="border border-[#987344] bg-[#3b2919] px-4 py-2 text-[8px] uppercase text-[#efd6a8]">Add Role</button>
                  </div>
                  <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
                    {ATTRIBUTE_FIELDS.map(({ key, label }) => (
                      <label key={key}>
                        <span className="mb-1 block text-[7px] uppercase text-[#756958]">{label}</span>
                        <input type="number" name={`${key}Modifier`} min={-10} max={10} defaultValue={0} className="w-full border border-[#60482e]/50 bg-[#100c09] px-2 py-2 text-center text-sm text-[#d7c4a5]" />
                      </label>
                    ))}
                  </div>
                </form>
