  ...
  return (
    <section className="mt-6 border border-[rgb(var(--sep-skin-c1,169_138_96))]/35 bg-[rgb(var(--sep-colour-15100d))] p-4 sm:p-5">
      ...
      <form action={createStorePostPurchaseOffer} className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <label><span className={label}>Trigger category</span><select name="trigger_category" className={field} defaultValue=""><option value="">—</option><option value="skin">Skin</option><option value="cosmetic">Cosmetic</option><option value="music">Music</option><option value="friend_list">Friend List</option><option value="private_location">Private Location</option><option value="bundle">Bundle</option></select></label>
        <label className="md:col-span-2"><span className={label}>Description</span><input name="description" className={field} /></label>
        <label><span className={label}>Valid for days</span><input name="valid_for_days" type="number" min="1" defaultValue="14" className={field} /></label>
        <label className="flex items-center gap-2 text-xs text-[rgb(var(--sep-skin-c2,211_194_170))]"><input name="is_active" type="checkbox" defaultChecked />Active</label>
