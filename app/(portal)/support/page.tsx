...
export default async function SupportPage(){
 ...
 const unread=await getTicketUnreadCounts({admin,userId:identity.userId,ticketIds:(tickets??[]).map(t=>t.id),audience:"player"});
 return <main className="p-5 sm:p-7 lg:p-9"><TicketLiveSync/><div className="mx-auto max-w-5xl">
  ...
  <div className="mt-8 space-y-3">{(tickets??[]).length===0?<div className="border border-[rgb(var(--sep-colour-60482e))]/45 p-8 text-center text-sm text-[rgb(var(--sep-colour-8f806d))]">You have no support tickets.</div>:(tickets??[]).map(t=>{const n=unread.get(t.id)??0;return <Link key={t.id} href={`/support/${t.public_reference}`} className={`relative block border p-5 transition ${n>0?"border-[rgb(var(--sep-colour-a87532))] bg-[rgb(var(--sep-colour-24190f))] shadow-[inset_3px_0_0_rgb(var(--sep-colour-c18a42))]":"border-[rgb(var(--sep-colour-60482e))]/45 bg-[rgb(var(--sep-colour-15100d))] hover:border-[rgb(var(--sep-colour-947047))]"}`}><Badge count={n}/><div className="flex justify-between gap-3"><div><div className="text-[8px] uppercase tracking-[0.16em] text-[rgb(var(--sep-colour-8c704b))]">{t.public_reference} · {t.category.replaceAll("_"," ")}</div><h2 className="mt-2 font-serif text-xl text-[rgb(var(--sep-colour-e2c99f))]">{t.subject}</h2></div><span className="h-fit border border-[rgb(var(--sep-colour-60482e))]/55 px-3 py-2 text-[8px] uppercase text-[rgb(var(--sep-colour-b7a083))]">{t.status.replaceAll("_"," ")}</span></div><p className="mt-3 text-[9px] text-[rgb(var(--sep-colour-756957))]">Opened {fmt(t.created_at)} · Updated {fmt(t.updated_at)}</p></Link>})}</div>
 </div></main>;
}
