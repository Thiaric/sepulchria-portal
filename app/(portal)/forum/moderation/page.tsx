  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <header className="border border-[#60482e]/45 bg-[#15100d]">
        <div className="border-b border-[#60482e]/35 bg-[#1a130e] px-5 py-6 sm:px-7">
        ...
        <dl className="grid grid-cols-2 divide-x divide-[#60482e]/30 bg-[#100c09]">
          <Statistic
      ...
      <section className="mt-6">
        {logs.length === 0 ? (
          <div className="border border-[#60482e]/45 bg-[#15100d] px-5 py-12 text-center">
            <p className="font-serif text-2xl text-[#cdb590]">
          ...
          <div className="space-y-4">
            {logs.map((log) => {
              ...
              return (
                <article
                  key={log.id}
                  className="border border-[#60482e]/45 bg-[#15100d]"
                >
                  <header className="flex flex-col gap-3 border-b border-[#60482e]/30 bg-[#19120e] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <span className="text-sm text-[#baa68a]">
                        {getProfileName(
                          moderator,
                        )}
                      </span>
                    ...
                    <time
                      ...
                      className="text-[8px] uppercase tracking-[0.14em] text-[#6f6251]"
                    >
                      {formatDate(
                        log.created_at,
                      )}
                  ...
                  <div className="space-y-4 px-5 py-5 sm:px-6">
                    <div>
                      <p className="text-[7px] uppercase tracking-[0.16em] text-[#665947]">
                        Discussion
                      </p>
                      ...
                      {topicUrl ? (
                        <Link
                          href={topicUrl}
                          className="mt-2 inline-block font-serif text-xl text-[#d4bb94] transition hover:text-[#f0d4a8]"
                        >
                          {topicTitle}
                        </Link>
                      ) : (
                        <p className="mt-2 font-serif text-xl text-[#968572]">
                          {topicTitle}
                        </p>
                      )}

                      {sectionName ? (
                        <p className="mt-2 text-xs text-[#786b5a]">
                          {sectionName}
                        </p>
                      ) : null}
                    </div>
                    ...
                    {reason ? (
                      <div className="border-l-2 border-[#755535] bg-[#100c09] px-4 py-3">
                        <p className="text-[7px] uppercase tracking-[0.16em] text-[#806b50]">
                          Moderation note
                        </p>

                        <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-[#aa9a83]">
                          {reason}
                        </p>
                      </div>
                    ) : null}
                    ...
                    {log.post_id ? (
                      <p className="text-[8px] uppercase tracking-[0.13em] text-[#625747]">
                        Action applied to
                        an individual post
                      </p>
                    ) : (
      ...
      {totalPages > 1 ? (
        <nav
          ...
          className="mt-7 flex items-center justify-between gap-4 border border-[#60482e]/40 bg-[#15100d] px-4 py-4"
        >
          {previousPage ? (
            <Link
              href={`/forum/moderation?page=${previousPage}`}
            ...
            <span className="border border-[#403426]/40 px-4 py-3 text-[8px] uppercase tracking-[0.16em] text-[#4f463b]">
              Previous
            </span>
          )}

          <span className="text-[8px] uppercase tracking-[0.16em] text-[#756653]">
            Page {currentPage} of{" "}
            {totalPages}
          </span>
          ...
          {nextPage ? (
            <Link
              href={`/forum/moderation?page=${nextPage}`}
              className="border border-[#60482e]/55 bg-[#19120e] px-4 py-3 text-[8px] uppercase tracking-[0.16em] text-[#a58b68] transition hover:border-[#947047] hover:text-[#dec095]"
            >
              Next
            </Link>
