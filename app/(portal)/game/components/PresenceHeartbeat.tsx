...
export default function PresenceHeartbeat({
  ...
  const [status, setStatus] =
    useState<PresenceStatus>(initialStatus);
  ...
  const updatePresence = useCallback(
    async (
      ...
      if (presenceError) {
        setError("Presence could not be updated.");
  ...
  const handleStatusChange = useCallback(
    async (nextStatus: PresenceStatus) => {
      manualStatusRef.current = nextStatus;

      await updatePresence(nextStatus, {
  ...
  useEffect(() => {
    void updatePresence(initialStatus);

    const heartbeat = window.setInterval(() => {
      void updatePresence(statusRef.current);
    }, HEARTBEAT_INTERVAL_MS);

    return () => {
      window.clearInterval(heartbeat);
    };
  }, [initialStatus, roomId, updatePresence]);
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      ...
      const restoredStatus =
        manualStatusRef.current === "busy"
          ? "busy"
          : "online";

      void updatePresence(restoredStatus);
    };
