...
export function MechanicalFeatPanel({
  ...
  const cooldown =
    Boolean(gift.cooldownUntil) &&
    Date.parse(gift.cooldownUntil as string) > Date.now();
  ...
  return (
    <div className="mt-3 border border-[rgb(var(--sep-colour-765937))]/45 bg-[rgb(var(--sep-colour-15100d))] p-3">
      ...
      <div className="grid gap-2 text-[9px] text-[rgb(var(--sep-colour-9e8b70))] sm:grid-cols-2 lg:grid-cols-4">
        <span>
          <b className="text-[rgb(var(--sep-colour-cdb48d))]">Target:</b>{" "}
          {targetMode} ·{" "}
          {shape.target_scope === "multiple"
            ? `up to ${maxTargets}`
            : "single"}
        ...
        <span>
          <b className="text-[rgb(var(--sep-colour-cdb48d))]">Nature:</b>{" "}
          {shape.effect_nature ?? "—"}
        </span>
        <span>
          <b className="text-[rgb(var(--sep-colour-cdb48d))]">Duration:</b>{" "}
          {durationLabel(shape)}
        </span>
      </div>

      {selfBits.length ? (
        <p className="mt-3 text-[9px] leading-5 text-[rgb(var(--sep-colour-a99577))]">
          <b className="text-[rgb(var(--sep-colour-d7bd91))]">Self profile:</b>{" "}
          {selfBits.join(" · ")}
        </p>
      ) : null}

      {otherBits.length ? (
        <p className="mt-1 text-[9px] leading-5 text-[rgb(var(--sep-colour-a99577))]">
          <b className="text-[rgb(var(--sep-colour-d7bd91))]">Other profile:</b>{" "}
          {otherBits.join(" · ")}
        </p>
      ) : null}

      {shape.other_alternative_enabled && harmfulBits.length ? (
        <p className="mt-1 text-[9px] leading-5 text-[rgb(var(--sep-colour-a99577))]">
          <b className="text-[rgb(var(--sep-colour-d7bd91))]">Harmful Other:</b>{" "}
          {harmfulBits.join(" · ")}
        </p>
      ) : null}
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          formAction={action}
          formNoValidate
          disabled={cooldown || targets.length === 0}
          className="border border-[rgb(var(--sep-colour-85653c))] bg-[rgb(var(--sep-colour-342617))] px-4 py-2.5 text-[8px] uppercase tracking-[0.14em] text-[rgb(var(--sep-colour-efd4a0))] transition hover:bg-[rgb(var(--sep-colour-4a351f))] disabled:cursor-not-allowed disabled:opacity-45"
        >
          {cooldown ? "On Cooldown" : "Use Feat"}
        </button>
      </div>
    </div>
  );
