...
export function WarpingPanel({presentCharacters,onBack}:{presentCharacters:C[];onBack:()=>void}){
 ...
 return <div className="border border-[#60482e]/45 bg-[#100c09] p-4">
  <div className="flex justify-between gap-3"><div><p className="text-[8px] uppercase tracking-[.2em] text-[#806b50]">The Current</p><h3 className="font-serif text-xl text-[#dfc99f]">Warping</h3></div><div className="text-[10px] text-[#a99577]"><span>Affinity <b className="text-[#ead1a3]">{r.affinity}</b></span><span className="ml-3">Warps <b className="text-[#ead1a3]">{r.warps_remaining} / {r.warps_per_day}</b></span><span className="ml-3 text-[#806b50]">Reset 08:00 UK</span> <button type="button" onClick={onBack} className="ml-3 border border-[#60482e]/55 px-3 py-2 text-[8px] uppercase">Back to Chat</button></div></div>
  {!r.shapes?.length?<p className="mt-4 text-xs text-[#8f8271]">You have no Shapes assigned.</p>:<>
   <select value={s?.id??""} onChange={e=>{setSid(e.target.value);setTargets([]);setDispelEffects([]);setSelectedDispelEffect("")}} className="mt-4 w-full border border-[#60482e]/55 bg-[#0f0c09] px-3 py-2 text-[10px] text-[#d8c29b]">{r.shapes.map((x:S)=><option key={x.id} value={x.id}>L{x.level} · {x.name} · {x.word_of_power}{x.level_available?"":" · LOCKED"}</option>)}</select>
   {s?<ShapeInformation shape={s}/>:null}
   {s?.target_mode==="written"?<input value={written} onChange={e=>setWritten(e.target.value)} placeholder="Written / Fate target..." className="mt-3 w-full border border-[#60482e]/55 bg-[#0f0c09] px-3 py-2 text-[10px]"/>:s?.target_mode!=="self"?<div className="mt-3 flex flex-wrap gap-2">{s?.target_mode==="either"&&(!blockedTargets.includes(r.character_id)||s?.is_dispel)?<button type="button" onClick={()=>toggle(r.character_id)} className="border border-[#60482e]/55 px-3 py-2 text-[9px]">Self</button>:null}{presentCharacters.filter(c=>c.id!==r.character_id&&(!blockedTargets.includes(c.id)||s?.is_dispel)).map(c=><button key={c.id} type="button" onClick={()=>toggle(c.id)} className={"border px-3 py-2 text-[9px] "+(targets.includes(c.id)?"border-[#b88b50] bg-[#2a1d12]":"border-[#60482e]/55")}>{c.display_name??c.displayName}</button>)}</div>:<p className="mt-3 text-[10px] text-[#9e8b70]">Target: Self · automatic success.</p>}
   {s?.is_dispel&&targets.length===1?<div className="mt-3 border border-[#60482e]/35 bg-[#15100d] p-3"><p className="text-[8px] uppercase tracking-[.14em] text-[#806b50]">Effect to Dispel</p>{dispelEffects.length?<select value={selectedDispelEffect} onChange={e=>setSelectedDispelEffect(e.target.value)} className="mt-2 w-full border border-[#60482e]/55 bg-[#0f0c09] px-3 py-2 text-[10px] text-[#d8c29b]"><option value="">Choose active effect...</option>{dispelEffects.map((e:any)=><option key={e.id} value={e.id}>{e.shape_name} · Level {e.shape_level} · {e.effect_nature}</option>)}</select>:<p className="mt-2 text-[9px] text-[#887865]">This character has no active effect that this Level {s.level} Dispel can remove.</p>}</div>:null}
   {access&&!access.allowed?<div className="mt-3 border border-[#6f493f]/60 bg-[#1b100d] px-3 py-2 text-[9px] text-[#d58d82]">Unavailable: {access.reasons.join(" · ")}</div>:null}
   <button type="button" disabled={busy||!access?.allowed||(Boolean(s?.is_dispel)&&!selectedDispelEffect)} onClick={()=>void warp()} className="mt-4 border border-[#9b7446] bg-[#2a1d12] px-5 py-2 text-[9px] uppercase text-[#ead1a3] disabled:opacity-40">{busy?"Warping...":`Warp ${s?.word_of_power??""}`}</button>
  </>}
