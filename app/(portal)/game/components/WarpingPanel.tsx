...
export function WarpingPanel({
  ...
  async function warp() {
    ...
    try {
      ...
      /*
       * Written/Fate targets do not
       * use character effect profiles.
       */
      const castProfiles =
        wt
          ? []
       ...
       *
       * Adds e.g.
       * Damage: 1d6 + Vigour Fire
       * to the Location Chat Warp
       * announcement.
       */
      if (!wt) {
          ...
          self
            ...
            : targets.map(
                id => {
                  ...
                  return s.other_alternative_enabled &&
                    (
                      targetEffectChoices[
                        id
                      ] ??
        ...
        for (const profile of uniqueEffectProfiles) {
          const damageDice =
            s[
      ...
      if (
        ...
        parts.push(
          `Duration: ${s.duration_amount} ${s.duration_unit}`,
        );
      } else if (
        s.duration_unit ===
        "until_dispelled"
      ...
      if (s.is_dispel) {
        ...
        parts.push(
          `Dispel Level ${s.level}`,
        );

        if (de) {
          parts.push(
            `Effect: ${de.shape_name} · Level ${de.shape_level}`,
          );
        }
      }
      ...
      if (
        ...
        parts.push(
          `Price: ${label} · Stage ${stage} · ${days} days`,
        );
      }
