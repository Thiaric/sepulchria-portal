...
export function WarpingPanel({presentCharacters,onBack}:{presentCharacters:C[];onBack:()=>void}){
 ...
 async function warp(){if(!s||busy)return;setBusy(true);setMsg("");try{
  ...
  const rows=wt
    ...
    : self
      ...
      : targets.map(id=>({
          ...
          outcome:"pending",
          resolved_at:null,
          other_effect_choice:s.other_alternative_enabled?otherEffectChoice:null,
        }));
  ...
  const profile = self ? "self" : (s.other_alternative_enabled&&otherEffectChoice==="harmful"?"other_alt":"other");
  ...
  setTargets([]);setWritten("");setTargetChoice("character");setOtherEffectChoice("beneficial");setSelectedDispelEffect("");setDispelEffects([]);setMsg(preparedDispelMessage||(wt?"Warp recorded. Fate resolves it manually.":"Shape warped."));await load()
 ...
 return <div className="border border-[rgb(var(--sep-colour-60482e))]/45 bg-[rgb(var(--sep-colour-100c09))] p-4">
  ...
  {!r.shapes?.length?<p className="mt-4 text-xs text-[rgb(var(--sep-colour-8f8271))]">You have no Shapes assigned.</p>:<>
   ...
   {s?.other_alternative_enabled&&targetChoice==="character"&&targets.some(id=>id!==r.character_id)?<div className="mt-3 border border-[rgb(var(--sep-colour-60482e))]/35 bg-[rgb(var(--sep-colour-15100d))] p-3"><p className="text-[8px] uppercase tracking-[.14em] text-[rgb(var(--sep-colour-806b50))]">Other Effect</p><div className="mt-2 flex gap-2"><button type="button" onClick={()=>setOtherEffectChoice("beneficial")} className={"border px-3 py-2 text-[8px] uppercase "+(otherEffectChoice==="beneficial"?"border-[rgb(var(--sep-colour-b88b50))] bg-[rgb(var(--sep-colour-2a1d12))]":"border-[rgb(var(--sep-colour-60482e))]/55")}>Beneficial</button><button type="button" onClick={()=>setOtherEffectChoice("harmful")} className={"border px-3 py-2 text-[8px] uppercase "+(otherEffectChoice==="harmful"?"border-[rgb(var(--sep-colour-b88b50))] bg-[rgb(var(--sep-colour-2a1d12))]":"border-[rgb(var(--sep-colour-60482e))]/55")}>Harmful</button></div></div>:null}
