...
const VALID_STAFF_SECTION_ROLES = [
  "admin",
  "moderator",
  "master",
] as const;

type StaffSectionRole =
  (typeof VALID_STAFF_SECTION_ROLES)[number];

function readStaffSectionRoles(
  formData: FormData,
  fieldName: string,
): StaffSectionRole[] {
  return Array.from(
    new Set(
      formData
        ...
        .filter(
          (value): value is StaffSectionRole =>
            VALID_STAFF_SECTION_ROLES.includes(
              value as StaffSectionRole,
            ),
        ),
    ),
  );
...
function validateSectionForm(
  ...
  if (
    visibility === "staff" &&
    staffWriteRoles.some(
      (role) => !staffReadRoles.includes(role),
    )
  ) {
    onError(
      "A staff role cannot have Write access without Read access.",
    );
  }
