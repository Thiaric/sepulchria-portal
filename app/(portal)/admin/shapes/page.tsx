...
export default async function AdminShapesPage({searchParams}:Props){
  ...
  return <main className="p-5 sm:p-7 lg:p-9"><div className="mx-auto max-w-7xl"><p className="text-[9px] uppercase tracking-[0.28em] text-[#8c704b]">Administration</p><h1 className="mt-2 font-serif text-4xl text-[#ead5ac]">Warping — Shapes</h1>
    ...
    <div className="mt-8 space-y-4">{shapes.map(s=><details key={s.id} id={`shape-${s.id}`} className="scroll-mt-6 border border-[#60482e]/45 bg-[#15100d]">
      ...
      <div className="border border-[#60482e]/35 p-4"><h3 className="font-serif text-lg text-[#d8c29b]">Order Level Shapes</h3><p className="mt-1 text-[9px] text-[#766a5b]">Members inherit Shapes from their current Order Level and every lower Level.</p><form action={linkOrderLevel} className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]"><input type="hidden" name="shape_id" value={s.id}/><select required name="order_level_id" className={cls}><option value="">Order Level...</option>{levels.filter((l:any)=>!(s.order_links??[]).some((x:any)=>x.order_level_id===l.id)).map((l:any)=><option key={l.id} value={l.id}>{l.orderName} - Level {l.level}</option>)}</select><button className="border border-[#765937] px-3 py-2 text-[8px] uppercase text-[#d6bb8d]">Link</button></form><div className="mt-3 space-y-1">{(s.order_links??[]).map((link:any)=>{const level=levels.find((entry:any)=>entry.id===link.order_level_id);return <form key={link.id??`${s.id}-${link.order_level_id}`} action={unlinkOrderLevel} className="flex items-center justify-between gap-3 border-t border-[#60482e]/25 pt-2"><input type="hidden" name="shape_id" value={s.id}/><input type="hidden" name="order_level_id" value={link.order_level_id}/><span className="text-[9px] text-[#8f8271]">{level?`${level.orderName} - Level ${level.level}`:"Unknown Order Level"}</span><button className="text-[8px] uppercase tracking-[0.1em] text-red-300">Unlink</button></form>})}{!(s.order_links??[]).length?<p className="text-[9px] italic text-[#746858]">Not linked to an Order Level.</p>:null}</div></div></div>
      </div>
    </details>)}</div>
  </div></main>;
}
