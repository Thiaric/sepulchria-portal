...
type DeletionRow = { message_id: string };
...
type ConversationCard = {
  ...
  matchedMessages: { id: string; body: string; createdAt: string }[];
};
...
function characterName(character: CharacterSummary): string {
  return (
    ...
    [character.first_name, character.surname].filter(Boolean).join(" ").trim() ||
    "Unnamed character"
  );
}
...
export default async function MessagesPage({ searchParams }: Props) {
  ...
  const { data: membershipRows = [], error: membershipsError } = await membershipQuery;
  if (membershipsError) throw new Error(membershipsError.message);

  const rows = membershipRows as unknown as ParticipantRow[];
  ...
  const [
    ...
    conversationIds.length
      ? supabase
          ...
          .select("id, conversation_id, body, created_at, sender_character_id")
          .in("conversation_id", conversationIds)
  ...
  const deletedMessageIds = new Set(
    ((deletionsResult.data ?? []) as DeletionRow[]).map((row) => row.message_id),
  );

  const visibleMessages = ((allMessagesResult.data ?? []) as DirectMessageRow[]).filter(
  ...
  const availableCharacters = (
    (availableCharactersResult.data ?? []) as CharacterSummary[]
  ).filter((candidate) => !blockedCharacterIds.has(candidate.id));

  const otherByConversation = new Map<string, CharacterSummary>();
  for (const row of (otherParticipantsResult.data ?? []) as unknown as (
    OtherParticipantRow & { conversation_id: string }
  )[]) {
    const other = normaliseRelation(row.character);
    if (other) otherByConversation.set(row.conversation_id, other);
  }

  const messagesByConversation = new Map<string, DirectMessageRow[]>();
  ...
  const conversations = rows
    .map<ConversationCard | null>((row) => {
      const conversation = normaliseRelation(row.conversation);
      ...
      const lastReadTime = row.last_read_at ? Date.parse(row.last_read_at) : 0;
      ...
      const name = other ? characterName(other) : "Unknown character";
      const searchableText = [
        name,
        other?.title,
        ...messages.map((message) => richTextToPlainText(message.body)),
      ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
      ...
      return {
        ...
        unreadCount,
        searchableText,
        matchedMessages: messages.map((message) => ({
          id: message.id,
          body: message.body,
    ...
    .filter((conversation): conversation is ConversationCard => conversation !== null)
