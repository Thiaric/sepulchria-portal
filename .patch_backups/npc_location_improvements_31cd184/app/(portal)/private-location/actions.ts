...
async function removePrivateLocationInvitationNotification(
  admin: ReturnType<
    typeof adminClient
  >,
  invitationId: string,
  _recipientCharacterId: string,
) {
  /*
   * Once the recipient accepts or refuses the invitation,
   * the notification has no further purpose.
   * Delete it outright, matching Daily Mission reward behaviour.
   */
  const { error } = await admin
    .from("notifications")
    .delete()
  ...
  if (error) {
    throw new Error(
      `Unable to remove the invitation notification: ${error.message}`,
    );
  }
}
...
export async function respondPrivateLocationInvitation(
  ...
  await removePrivateLocationInvitationNotification(
    admin,
    invitationId,
    character.id,
  );
