...
function themeFrom(
  ...
  return {
    ...
    whisperBackgroundColour:
      row.whisper_background_colour,
    whisperTextColour:
      row.whisper_text_colour,
    offgameBackgroundColour:
      row.offgame_background_colour,
    offgameTextColour:
      row.offgame_text_colour,
  };
}
...
async function getMembershipLevel(
  ...
  const { data, error } = await admin
    ...
    .select(
      `level:order_levels!order_memberships_order_level_id_fkey(level)`,
    )
    .eq(
      "character_id",
      characterId,
  ...
  if (error) {
    throw new Error(
      `Unable to verify Order membership: ${error.message}`,
    );
  }

  return (
    asOne(data?.level ?? null)
      ?.level ?? null
  );
}
...
export async function getOrderHeadquartersAccess(
  ...
  if (error) {
    throw new Error(
      `Unable to inspect Order Headquarters: ${error.message}`,
    );
  }
...
export async function getOrderHeadquartersVisibility(
  characterId: string,
) {
  const admin =
    adminClient();
...
export async function getOrderHeadquartersManageData(
  ...
  /*
   * Pending invitations are always shown.
   *
   * Accepted invitations are shown only while they are still valid.
   * Permanent invitations have expires_at = null.
   *
   * Once a temporary accepted invitation expires, it disappears from
   * Manage Headquarters and the character becomes inviteable again.
   */
  const now =
    Date.now();
  ...
  /*
   * Only CURRENT pending/accepted guests are removed from the invitation
   * candidate list. Expired guests therefore become available to invite
   * again automatically.
   */
  const inviteeIds =
    new Set(
      activeInvitations.map(
        (i) =>
          i.recipient_character_id as string,
      ),
    );
