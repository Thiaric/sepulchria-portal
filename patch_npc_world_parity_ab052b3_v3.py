...
ACTIVE_CONTROLS_OLD='{!creating?<label className="mt-3 flex items-center gap-2 text-[8px] uppercase tracking-[0.12em] text-[rgb(var(--sep-colour-8f8170))]"><input type="checkbox" checked={active} onChange={e=>setActive(e.target.checked)}/>Active</label>:null}'
...
def patch_city(s):
    ...
    s=rep(s,'href: `/characters/${person.public_slug}`,','href: person.is_system?`/npcs/${person.id}`:`/characters/${person.public_slug}`,',"city NPC route",3)
    return s
