...
def restore_clean_backup():
    if not BACKUP.exists(): die(f"No clean backup found at {BACKUP}")
    print("Restoring clean pre-live-mechanics files from backup...")
    for p in FILES:
        src=BACKUP/p
        if not src.exists(): die(f"Backup is missing {p}")
        dst=ROOT/p
        dst.parent.mkdir(parents=True,exist_ok=True)
        shutil.copy2(src,dst)
    if (ROOT/NEWFILE).exists(): (ROOT/NEWFILE).unlink()
...
def main():
    ...
    print("Revert code: python patch_npc_live_mechanics_v2.py --revert")
