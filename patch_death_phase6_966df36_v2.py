...
# Insert the status block immediately after the utility controls conditional.
# This avoids depending on exact whitespace at the end of the component.
chat_path = "app/(portal)/game/components/RoomChatForm.tsx"
chat = files[chat_path]

controls_marker = "data-room-chat-controls"
controls_index = chat.find(controls_marker)

if controls_index == -1:
    fail("Could not locate data-room-chat-controls in RoomChatForm.tsx.")

controls_end = chat.find("      ) : null}", controls_index)

if controls_end == -1:
    fail("Could not locate the end of the utility controls conditional.")

controls_end += len("      ) : null}")
