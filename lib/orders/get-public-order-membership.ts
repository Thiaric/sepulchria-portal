...
export async function getPublicOrderMembership(
  ...
  if (error) {
    throw new Error(`Unable to load public Order membership: ${error.message}`);
  }
  ...
  return {
    ...
    association: one(order.association),
    level: one(data.level as Relation<{
      id: string;
      level: number;
      name: string | null;
    }>),
    job: one(data.job as Relation<{
      id: string;
      name: string;
    }>),
  };
