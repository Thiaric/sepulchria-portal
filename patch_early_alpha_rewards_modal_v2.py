...
# 1. Add infoOpen state if needed.
if "const [infoOpen, setInfoOpen] = useState(false);" not in text:
    anchor = '  const [open, setOpen] = useState(false);\n'
    if anchor not in text:
        raise RuntimeError(
            "Could not find the existing open state. No files were written."
        )

    text = text.replace(
        anchor,
        anchor + '  const [infoOpen, setInfoOpen] = useState(false);\n',
        1,
    )

# 2. Make Escape/body-lock work for both modals.
...
elif "[open, infoOpen, loading]" not in text:
    raise RuntimeError(
        "Could not safely update the modal Escape/body-lock effect. No files were written."
    )

# 3. Replace the Alpha paragraph robustly, regardless of whitespace.
replacement_paragraph = '''<p className="mx-auto mt-3 max-w-xl text-xs leading-6 text-[rgb(var(--sep-colour-8f8271))] components_registration_closed_notice_p_closed_alpha_access_3">
...
if "onClick={() => setInfoOpen(true)}" not in text:
    paragraph_pattern = re.compile(
        r'<p className="mx-auto mt-3 max-w-xl text-xs leading-6 text-\[rgb\(var\(--sep-colour-8f8271\)\)\] components_registration_closed_notice_p_closed_alpha_access_3">\s*'
        r'Applications are reviewed individually and invitations are limited\..*?'
        r'</p>',
        re.DOTALL,
    )

    text, count = paragraph_pattern.subn(
    ...
    if count != 1:
        raise RuntimeError(
            "Could not find the Alpha applications paragraph. No files were written."
        )

# 4. Insert the info modal before the existing application modal.
info_modal = '''      {infoOpen ? (
        <div
...
if 'aria-labelledby="alpha-rewards-title"' not in text:
    app_modal_anchor = '''      {open ? (
        <div
'''
    pos = text.find(app_modal_anchor)

    if pos == -1:
        raise RuntimeError(
            "Could not find the existing application modal. No files were written."
        )

    text = text[:pos] + info_modal + text[pos:]

# 5. Create the independently editable page if missing.
...
for marker in (
    ...
    if marker not in text:
        raise RuntimeError(
            f"Validation failed: {marker!r} is missing. No files were written."
        )
...
print("Early Alpha rewards modal patch applied successfully.")
print()
print("Changed:")
print("  components/registration-closed-notice.tsx")
print()
print("Created if missing:")
print("  app/early-alpha-rewards/page.tsx")
print()
print("Now run:")
print("  npm run build")
