...
export function WarpingPanel({presentCharacters,onBack}:{presentCharacters:C[];onBack:()=>void}){
 ...
 return <div className="border border-[#60482e]/45 bg-[#100c09] p-4">
  ...
  {!r.shapes?.length?<p className="mt-4 text-xs text-[#8f8271]">You have no Shapes assigned.</p>:<>
   <select value={s?.id??""} onChange={e=>{setSid(e.target.value);setTargets([]);setDispelEffects([]);setSelectedDispelEffect("")}} className="mt-4 w-full border border-[#60482e]/55 bg-[#0f0c09] px-3 py-2 text-[10px] text-[#d8c29b]">{r.shapes.map((x:S)=><option key={x.id} value={x.id}>L{x.level} · {x.name} · {x.word_of_power}</option>)}</select>
   ...
   <button type="button" disabled={busy||r.warps_remaining<=0||(Boolean(s?.is_dispel)&&!selectedDispelEffect)} onClick={()=>void warp()} className="mt-4 border border-[#9b7446] bg-[#2a1d12] px-5 py-2 text-[9px] uppercase text-[#ead1a3] disabled:opacity-40">{busy?"Warping...":`Warp ${s?.word_of_power??""}`}</button>
  </>}
