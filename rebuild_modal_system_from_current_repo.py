...
SIDEBAR_REL = "components/portal/portal-sidebar.tsx"
BUTTON_REL = "components/portal/portal-modal-button.tsx"

SIDEBAR = ROOT / SIDEBAR_REL
BUTTON = ROOT / BUTTON_REL
...
if not (ROOT / ".git").exists():
    fail("Run this from the Sepulchria repository root.")
...
try:
    ...
    fail("Unable to fetch origin/master.")
...
try:
    ...
    button = subprocess.check_output(
        [
            "git",
            "show",
            f"origin/master:{BUTTON_REL}",
        ],
        cwd=ROOT,
        text=True,
        encoding="utf-8",
    ...
    fail(
        "Unable to read the modal files from origin/master."
    )

if "function PublicPageModal({" not in sidebar:
    fail(
        "origin/master does not contain the expected PublicPageModal."
    )

if "export function openPortalModal" not in button:
    fail(
        "origin/master does not contain the expected openPortalModal."
    )

old_open = '''export function openPortalModal(payload: PortalModalPayload) {
...
new_open = '''export function openPortalModal(payload: PortalModalPayload) {
...
if button.count(old_open) != 1:
    fail(
        "The pushed portal-modal-button.tsx does not match the expected current implementation."
    )
...
listener_start_token = '''  useEffect(() => {
    /*
     * A page rendered inside a portal modal still mounts the portal layout,
'''

listener_end_token = '''  function renderNavigationItem(
'''

listener_start = sidebar.find(listener_start_token)
listener_end = sidebar.find(
    listener_end_token,
    listener_start,
)
...
if (
    listener_start == -1
    or listener_end == -1
    or listener_end <= listener_start
):
    fail(
        "Could not locate the pushed external modal listener."
    )
...
sidebar = (
    sidebar[:listener_start]
    + new_listener
    + sidebar[listener_end:]
)

modal_start_token = "function PublicPageModal({"
modal_end_token = "function NavigationGroup({"

modal_start = sidebar.find(modal_start_token)
modal_end = sidebar.find(
    modal_end_token,
    modal_start,
)
...
if (
    modal_start == -1
    or modal_end == -1
    or modal_end <= modal_start
):
    fail(
        "Could not locate the pushed PublicPageModal boundaries."
    )

new_modal = '''function PublicPageModal({
  item,
  onClose,
}: {
  item: NavigationItem;
...
for marker in required:
    if marker not in sidebar:
        fail(
            f"Rewrite safety check failed: missing {marker!r}"
        )
...
for forbidden in [
    ...
    if forbidden in sidebar:
        fail(
            f"Old modal code still remains after rewrite: {forbidden!r}"
        )

if "window.parent.postMessage(" not in button:
    fail(
        "Rewrite safety check failed: iframe postMessage is missing."
    )
...
print()
print("MODAL SYSTEM REBUILT FROM CURRENT origin/master")
print()
print("Clean baseline restored from origin/master for:")
print(f"- {SIDEBAR_REL}")
print(f"- {BUTTON_REL}")
print()
print("Replacement:")
print("- one top-level modal owner")
print("- one modal at a time")
print("- iframe requests replace, never stack")
print("- explicit React x/y/width/height")
print("- dedicated pointer resize handle")
print("- no browser-native CSS resize")
print("- no ResizeObserver")
print("- stable collapse / exact restore")
print("- no 1279px portal mobile-shell switch")
print("- stable one-column content below 900px")
print("- no anonymous iframe event bridge")
print()
print("Unrelated files were not touched.")
print()
