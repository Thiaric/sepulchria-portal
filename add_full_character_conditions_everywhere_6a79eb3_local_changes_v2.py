...
import {
  ...
  presentCharacters: PresentRoomCharacter[];''',
    ...
    # Insert the Conditions editor immediately after PendingShapeResponses.
    # This tolerates local layout changes around the composer.
    import re

    location_editor = '''      <CharacterConditionsEditor
        scope="location"
    ...
    if "<CharacterConditionsEditor" not in text:
        matches = list(
            re.finditer(
                r'(?m)^(\s*)<PendingShapeResponses\s*/>\s*$',
                text,
            )
        )
        ...
        if len(matches) != 1:
            raise RuntimeError(
                "RoomChatForm: could not uniquely locate <PendingShapeResponses /> "
        ...
        rendered_editor = "\n\n".join(
            line if not line else indent + line.lstrip()
            for line in location_editor.splitlines()
        )

        # Keep the exact internal formatting from the template, only align
        # the first line with the surrounding JSX indentation.
        rendered_editor = "\n".join(
            (
                indent + line
                if index == 0
...
print("")
print("Local tracked changes were allowed.")
print("The patch only changes files where the exact 6a79eb3 target anchors still match.")
print("")
print("CHARACTER:")
