...
TROPHIES_COMPONENT = ROOT / "components/characters/character-trophies-display.tsx"
...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    ...
    if count == 0:
        fail(
            f"Could not find expected anchor for {label}. "
            "Your local file may differ from commit 78f6c35."
        )
    if count > 1:
        fail(
            f"Expected one anchor for {label}, found {count}. "
            "Patch stopped rather than guessing."
        )
...
def patch_tabs() -> None:
  ...
  | "gifts"''',
    ...
    if '{ id: "trophies", label: "TROPHIES" }' not in text:
        text = replace_once(
            text,
  ...
  { id: "gifts", label: "FEATS" },''',
    ...
    trophies_selector = (
        '.character-sheet-tabs[data-character-sheet-active-tab="trophies"] '
        '[data-character-sheet-panel="trophies"],'
    )

    if trophies_selector not in text:
        text = replace_once(
            text,
        ...
        .character-sheet-tabs[data-character-sheet-active-tab="gifts"] [data-character-sheet-panel="gifts"],''',
            "TROPHIES panel selector",
        )

    write(TABS, text)


TROPHIES_TSX = r'''import "server-only";
...
def create_trophies_component() -> None:
    if TROPHIES_COMPONENT.exists():
        existing = read(TROPHIES_COMPONENT)
        if "export async function CharacterTrophiesDisplay" in existing:
            print(
                "SKIPPED: components/characters/character-trophies-display.tsx "
                "(already appears patched)"
            )
            return
        fail(
            "components/characters/character-trophies-display.tsx already exists "
            "but is not this patch's component. Refusing to overwrite it."
        )

    write(TROPHIES_COMPONENT, TROPHIES_TSX)


def patch_own_page() -> None:
    text = read(OWN_PAGE)

    import_line = (
        'import { CharacterTrophiesDisplay } '
        'from "@/components/characters/character-trophies-display";'
    )
...
import { LiveCharacterSheetRefresh } from "@/components/characters/live-character-sheet-refresh";''',
            "own character Trophy import",
        )

    if 'data-character-sheet-panel="trophies"' not in text:
        text = replace_once(
            text,
          ...
          <div data-character-sheet-panel="gifts">''',
            "own character Trophy panel",
        )

    write(OWN_PAGE, text)


def patch_public_profile() -> None:
    text = read(PUBLIC_PROFILE)
...
import { CharacterTrophiesDisplay } from "@/components/characters/character-trophies-display";
import { CharacterExpertiseTotal } from "@/components/characters/character-expertise-total";''',
            "public character Trophy import",
        )
    ...
    if 'data-character-sheet-panel="trophies"' not in text:
        ...
        <div data-character-sheet-panel="gifts">''',
            "public character Trophy panel",
        )

    write(PUBLIC_PROFILE, text)
...
def main() -> None:
    ...
    print("Sepulchria Trophy UI patch")
    print("Reference commit: 78f6c35")
    print("No GitHub/Vercel operations are performed.\n")

    patch_tabs()
    create_trophies_component()
    patch_own_page()
    patch_public_profile()

    print("\nSUCCESS: Trophy UI patch applied.")
    print("\nChanged/created:")
    print("  - components/characters/character-sheet-tabs.tsx")
    print("  - components/characters/character-trophies-display.tsx")
    print("  - app/(portal)/character/page.tsx")
    print("  - components/characters/public-character-profile.tsx")
    print("\nNext:")
    print("  1. npm run build")
    print("  2. Test /character -> TROPHIES")
    print("  3. Test /characters/<slug> -> TROPHIES")
    print("\nPublic profiles show earned Trophies only.")
    print("Own /character shows earned + locked progress.")
